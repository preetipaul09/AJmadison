...
# from modules.runTimeSecrets import HOST, DB, USER, PASS, HOST3, DB3, PASS3, USER3
...
#                 WHERE ProductVendor.vendor_id = %s AND VendorURL.vendor_url = "https://www.ajmadison.com/cgi-bin/ajmadison/WFG525S0JB.html"
#                 limit 1
#             """
...
#                     vendor_product_id, product_id, product_mpn, url = value[0], value[1], value[2].strip(), value[3].strip()
#                     if "html&" in url: 
...
#         mpn_element = driver.find_element(By.CSS_SELECTOR, 'div.container.container--fluid')
#         scraped_mpn = mpn_element.get_attribute("data-sku")

#         if scraped_mpn == product_mpn:
#             temp['vendor_product_id'] = vendor_product_id
#             temp['vendorprice_price'] = None
#             temp['vendorprice_finalprice'] = None
#             temp['msrp'] = None
            
#             basePice = driver.find_element(By.CSS_SELECTOR, 'div[itemprop="price"] span')
#             price_text = basePice.text.strip() if basePice else ""
#             if '-' in price_text:
#                 print(f"Skipping product due to base price range: {price_text}")
#                 return None, None
#             elif price_text:
#                 base_price = price_text.replace("$", "").replace(",", "").replace("Rs.", "").replace(r"\ea", "").strip()
#             else:
#                 base_price = None 

#             # And list price is the MSRP
#             try:
#                 msrp_element = driver.find_element(By.CSS_SELECTOR, 'td[itemprop="priceSpecification"] del')
#                 product_msrp = msrp_element.text.replace("$","").replace(",","").strip()
#             except:
#                 product_msrp  = None

#             # Assign to temp and temp as needed
...
#         else:
#             print("Product mpn not matched")
#             temp = {}
#             temp2 = {}
#             #searchbar logic ...................
#             url = f"https://www.ajmadison.com/cgi-bin/ajmadison/{product_mpn}.html"
#             driver.get(url)
#             time.sleep(10)
#             # driver.refresh()
#             # random_pause(15,20)
...
#             try:
#                 name_element = driver.find_element(By.CSS_SELECTOR, 'div.divided div.block')
...
#             brand_element = driver.find_element(By.CSS_SELECTOR, 'h1.pdpTitle div')
#             brand = brand_element.text.strip()

#             temp["brand_name"] = brand
#             temp["product_mpn"] = product_mpn
#             temp['vendor_sku'] = product_mpn
#             temp["product_url"] = url
#             temp2['url'] = url
#             # Image
#             try:
#                 img_element = driver.find_element(By.CSS_SELECTOR, "div.product-image2-main-image-container img")
#                 temp["product_image"] = img_element.get_attribute("src")
#             except:
...
#                 base_price = price_element.text.replace("$","").replace(",","").strip()
#             except:
#                 base_price = None
...
#             try:
#                 msrp_element = driver.find_element(By.CSS_SELECTOR, 'td[itemprop="priceSpecification"] del')
#                 msrp = msrp_element.text.replace("$","").replace(",","").strip()
#             except:
#                 msrp  = None
...
#             DBFunctionsForDirectScraping(temp,temp2,vendor_id)
#             # ✅ Insert into DB
...
#             #     logger.debug("vendorprice_price not found!!")
#             #     return
...
def getUrls(vendor_id, vendor_url):
    ...
    try:
        ...
        if conn_1.is_connected():
            ...
            getProductIds = """
                SELECT DISTINCT(121PreGeneratedReportsData.product_id)
                    ...
                    INNER JOIN 121PreGeneratedReports ON 121PreGeneratedReports.generated_report_id = 121PreGeneratedReportsData.generated_report_id
                    WHERE 121PreGeneratedReports.base_vendor_id = %s OR 121PreGeneratedReports.competitor_vendor_id = %s
            """
        ...
        if conn.is_connected() and productIds:
            ...
            INNER JOIN Product ON Product.product_id = ProductVendor.product_id
            INNER JOIN Brand ON Brand.brand_id = Product.brand_id	
            WHERE ProductVendor.vendor_id = %s AND VendorURL.vendor_url NOT IN ('https://www.ajmadison.com/cgi-bin/ajmadison/HMC54151UC.html')
            ...
            if url_list:
                logger.info(f"Found {len(url_list)} URLs to process")
                for vendor_product_id, product_id, product_mpn, url in url_list:
                    if product_mpn:
...
def scraper_unit(vendor_product_id, product_id, url, vendor_url, vendor_id, product_mpn):
    try:
       ...
       # Try multiple selectors for MPN
        ...
        #         if attr == 'textContent':
        #             element = elements[0]
        #             scraped_mpn = element.text.strip()
        #             if not scraped_mpn:  # If .text returns nothing, check the pseudo-element
        #                 try:
        #                     # JavaScript to get the content of the ::before pseudo-element
        ...
        #                     # Strip any leading/trailing quotes and whitespace from the result
        #                     scraped_mpn = pseudo_content.strip().replace('"', '')
        #                 except Exception:
        #                     scraped_mpn = None

        # if not scraped_mpn:
        #     logger.warning(f"No MPN found for product ID {product_id}")
        #     return None

        # if scraped_mpn.strip() != (product_mpn).strip():
        #     logger.warning(f"MPN mismatch: scraped={scraped_mpn}, expected={product_mpn}")
        #     return None

        # ✅ continue with your pricing logic
        # print(f"Scraped MPN : {scraped_mpn}")
        # print(f"Product_mpn : {product_mpn}")
        temp['vendor_product_id'] = vendor_product_id
        temp['vendorprice_price'] = None
        temp['vendorprice_finalprice'] = None
        temp['msrp'] = None
...
def triggerSelenium_chrome(useVPN=False, checkIP=False, config_path="vpn.config.json"):
    ...
    driver = uc.Chrome(version_main=139, options=options, seleniumwire_options=seleniumwire_options)
